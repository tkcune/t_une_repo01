...
var clipboard = function () {
  ...
  var nodeId = null; //@var string nodeDir コピーしているノードディレクトリ

  var nodeDir = null; //@var string selectNodeId 選択しているノードのid
  ...
  var copyNode = function copyNode(dir, id) {
    //コピーデータを代入する
    nodeDir = dir;
    nodeId = id;
  }; //選択したノードのディレクトリを返す
  ...
  var getCopyDir = function getCopyDir() {
    return nodeDir;
  }; //コピーノードのidを返す


  var getCopyId = function getCopyId() {
    return nodeId;
  }; //カレントのディレクトリを返す
...
TreeAction.node = /*#__PURE__*/function () {
  ...
  _createClass(Node, [{
    ...
    value: function createLeafTree() {
      ...
      var li = document.createElement('li'); //li要素に、タイトルを挿入する

      li.innerText = this.title; //クリック処理。テスト用のクリック処理
      //クリックした要素を表示する
    ...
    key: "insertTree",
    value: function insertTree(element, deleteId) {
      //@var 挿入する基準となるdom
      var insertElement; //基準のidがゼロ以上(最初以外)は、その前のdomを取得して、後ろにdomを挿入する
    ...
    key: "deleteElement",
    value: function deleteElement(deleteId) {
      if (this.className === 'expandtree' || this.className === 'lastexpandtree') {
    ...
    key: "resettingClassElement",
    value: function resettingClassElement() {
    ...
    value: function addExpandEvent(div) {
      ...
      div.addEventListener('click', function (event) {
        //+-文字の変化
        _this2.ChangeBox(); //ツリーのhtmlの表示、非表示を切り替える


        _this2.changeChildDispaly();
      });
    } //titleboxの生成
    ...
    value: function createTitleBox() {
      ...
      div.classList.add('titlebox'); //boxValueの1番目に、表示する文字。
    ...
    value: function openBottomUpTree() {
      ...
      if (splitDir.length !== 1) {
        ...
        palent.openBox();
        palent.openDisplayNode(); //また親要素を引数にして再帰的に、openBottomUpTreeを呼び出す
    ...
    value: function noneDisplayNode() {
      ...
      var node = this.element.children[1].children[0].children;
      ...
      for (var i = 0; i < node.length; i++) {
        ...
        if (node[i].classList.value.match('expandtree') || node[i].classList.value.match('lastexpandtree')) {
          ...
          var childNodes = node[i].childNodes; //展開するボックスの子要素(treeboxなど)を表示、非表示にする

          for (var _i = 0; _i < childNodes.length; _i++) {
            childNodes[_i].classList.add('unexpand');
          }
        } else {
          //ul要素の場合
      ...
      for (var i = 0; i < node.length; i++) {
        ...
        if (node[i].classList.value.match('expandtree') || node[i].classList.value.match('lastexpandtree')) {
          ...
          for (var _i2 = 0; _i2 < childNodes.length; _i2++) {
            childNodes[_i2].classList.remove('unexpand');
          }
        } else {
          //ul要素の場合
    ...
    value: function ChangeBox() {
      ...
      var box = this.element.children[0].children[0]; //プラスマイナス文字を反転する。
    ...
    value: function changeChildDispaly() {
      this.child.forEach(function (child) {
        ...
        if (child.element.classList.value.match('expandtree') || child.element.classList.value.match('lastexpandtree')) {
          var childNodes = child.element.childNodes; //展開するボックスの子要素(treeboxなど)を表示、非表示にする
    ...
    key: "deepCopyNode",
    value: function deepCopyNode() {
      //@var Nodeクラス copyNode コピーノードクラス
      //一からノードクラスを作成していく
      var copyNode = new Node(this.dir, this.id);
      copyNode.prototype = {
        chainparser: this.prototype.chainparser,
        tree: this.prototype.tree
      };
      copyNode.className = this.className; //dom要素は参照なので、cloneNodeで値コピーする

      copyNode.element = this.element.cloneNode(true); //投影元と投影先のディレクトリをコピーする。

      copyNode.toLink = [];
      this.toLink.forEach(function (link) {
        //linkの配列は参照なので、concatで値コピーしている
        copyNode.toLink.push(link.concat());
      });
      copyNode.fromLink = [];
      this.fromLink.forEach(function (link) {
        //linkの配列は参照なので、concatで値コピーしている
      ...
      if (copyNode.element.children[0].className.includes("treebox") === true) {
        ...
        var expandboxDiv = copyNode.element.children[0].children[0]; //ボックスの展開のクリックイベントを付ける

        expandboxDiv.node = copyNode;
        copyNode.addExpandEvent(expandboxDiv); //@var dom titleboxDiv 展開するボックスのタイトル

        var titleboxDiv = copyNode.element.children[0].children[1]; //タイトルの表示イベントを付ける

        titleboxDiv.addEventListener('click', {
          node: copyNode,
          handleEvent: this.displayDetail
        });
      } else {
        ...
        var li = copyNode.element.children[0]; //クリック処理。テスト用のクリック処理
        //クリックした要素を表示する
      ...
      this.child.forEach(function (child) {
        //子要素のコピーを作成して、追加
        copyNode.child.push(child.deepCopyNode()); //@var dom li 子要素のdomの親要素はli

        var li = copyNode.element.children[1].children[0];

        for (var i = 0; i < copyNode.child.length; i++) {
...
TreeAction.chainparser = function () {
  ...
  var exceptSepalete = function exceptSepalete(sepalete) {
    //@var array list 
    var list = [];
  ...
  var searchNodeDirId = function searchNodeDirId(nodeDir, nodeId, node) {
    //@var array nodeArray 展開したノードクラスを格納した配列
    var nodeArray = concatNode(node); //@var Nodeクラス search 検索するノードを代入する
  ...
  var searchPalentTitle = function searchPalentTitle(node) {
    //@var array nodeDir ノードのディレクトリを'/'を区切りに配列にした
    var nodeDir = node.dir.split('/'); //nodeのタイトルを配列から削除。

    nodeDir.pop(); //次に最後尾から取ったものが、親のタイトル
  ...
  var syncLinkNode = function syncLinkNode(fromNodeId, toNode, tree) {
    //@var Nodeクラス fromNode 投影元のノードクラス
    var fromNode = searchNodeId(fromNodeId, tree); //リンク元がない。投影先ではない

    if (fromNode.fromLink.length === 0) {
      //@var array concatFromNode リンク元を一列に展開
      //@var array concatToNode リンク先を一列に展開
      var concatFromNode = concatNode(fromNode);
      var concatToNode = concatNode(toNode);

      for (var i = 0; i < concatFromNode.length; i++) {
      ...
      fromNode.fromLink.forEach(function (fromLinkIdDir) {
        //@var Nodeクラス fromLinkNode 投影元のノードクラス
        var fromLinkNode = searchNodeDirId(fromLinkIdDir[1], fromLinkIdDir[0], tree); //@var array concatFromNode リンク元を一列に展開
        //@var array concatToNode リンク先を一列に展開

        var concatFromNode = concatNode(fromLinkNode);
        var concatToNode = concatNode(toNode);

        for (var _i3 = 0; _i3 < concatFromNode.length; _i3++) {
          //リンク元にリンク先のディレクトリを入れる
          //リンク先にリンク元のディレクトリを入れる
          syncLink(concatFromNode[_i3], concatToNode[_i3]);
        }
      });
    }
  ...
  var recreateDir = function recreateDir(node, nodeDir) {
  ...
  var recreateId = function recreateId(node, nodeChain) {
    //ノードチェインについてすべてループする
    nodeChain.forEach(function (chain) {
  ...
  var getTwoPalent = function getTwoPalent(node, tree) {
    //@var array splitDir ディレクトリを分けた配列
    var splitDir = node.dir.split('/'); //最後の文字を削除することによって、親要素のディレクトリになる

    splitDir.pop(); //@vaar Nodeクラス palent 親要素のディレクトリ

    var palent = searchNodeDir(splitDir.join('/'), tree); //親要素のディレクトリから最後の文字を削除することによって、親要素の親要素ディレクトリとなる

    splitDir.pop(); //@var Nodeクラス palentPalent 親要素の親要素のディレクトリ

    var palentPalent = searchNodeDir(splitDir.join('/'), tree);
...
TreeAction.createTree = function (treesepalete, projectionChain, Node, chainparser) {
  ...
  var createTopNode = function createTopNode(sepalete, treeTop) {
    ...
    var topNode; //親要素のchain(例{'1.chaintree', '4.作業'})を取り除いた配列

    var exceptSepalete = chainparser.exceptSepalete(sepalete); //topNodeを作成する
    ...
    parse(topNode, exceptSepalete); //Nodeクラスの第一階層のcss名を決定する。
  ...
  var syncProjection = function syncProjection(projectionChain) {
    projectionChain.forEach(function (chain) {
      //@var Nodeクラス fromNode 投影元のノードクラス
      var fromNode = chainparser.searchNodeId(Object.keys(chain)[0].split('.')[0], treeTop); //@var Nodeクラス toNode 投影先のノードクラス

      var toNode = chainparser.searchNodeId(Object.values(chain)[0].split('.')[0], treeTop); //投影先を斜体にする

      toNode.onSync(); //投影先と投影元をリンクさせる
  ...
  var treeTop = new Node('', 1); // treeTop.prototype.chainparser = chainparser;
  ...
  treeTop.className = 'chaintree'; //トップツリーの子要素のクラスを作成して、追加。
  ...
  decisionClass(treeTop); //domを生成して、ツリーを描画する

  createElement(treeTop); //ツリーのdom要素を生成した時は、ツリーは表示しているので、非表示にする。

  treeTop.child.forEach(function (child) {
    child.noneDisplayTree();
  }); //投影

  syncProjection(projectionChain); //ツリーインスタンスを返す
...
TreeAction["delete"] = function (chainparser, tree) {
  ...
  //削除機能
  var deleteNode = function deleteNode(nodeDir, nodeId) {
    //@var Nodeクラス node 削除されるノードクラス
    var node = chainparser.searchNodeDirId(nodeDir, nodeId, tree); //var array concatDeleteNode 削除されるノードクラスとそのノードクラスの子要素を一列に並べた配列

    var concatDeleteNode = chainparser.concatNode(node); //削除されるノードクラスに投影元がある場合について
    //投影元のノードクラスのプロパティtoLink(投影先のディレクトリ)を削除する

    var _loop = function _loop(i) {
      //@var array fromLink 投影元のディレクトリ
      var fromLink = concatDeleteNode[i].fromLink[0];

      if (fromLink) {
        //@var Nodeクラス fromLinkNode 投影元のノードクラス
        var fromLinkNode = chainparser.searchNodeId(fromLink, tree); //投影元のノードクラスから、投影先のディレクトリのデータを削除する

        fromLinkNode.toLink = fromLinkNode.toLink.filter(function (link) {
          return link === concatDeleteNode[i].id !== true;
        });
      }
    };

    for (var i = 0; i < concatDeleteNode.length; i++) {
      _loop(i);
    } //削除されるノードクラスに投影先がある場合について
    //投影元が削除されるので、投影先のノードクラスも削除する


    for (var _i4 = 0; _i4 < concatDeleteNode.length; _i4++) {
      //@var array toLink 投影先のディレクトリ配列
      var toLink = concatDeleteNode[_i4].toLink;

      if (toLink) {
        //投影先は複数ある場合があるので、ループする
        toLink.forEach(function (link) {
          //@var Nodeクラス toLinkNode 投影先のノードクラス
          var toLinkNode = chainparser.searchNodeId(link, tree);
  ...
  var deleteTreeNode = function deleteTreeNode(node) {
    ...
    var twoPalent = chainparser.getTwoPalent(node, tree); //@var Nodeクラス palent 削除するノードクラスの親要素

    var palent = twoPalent[0]; //@var Nodeクラス palentPalent 削除するノードクラスの親要素の親要素

    var palentPalent = twoPalent[1]; //@var int deleteId domの中で削除するノードクラスの番号

    var deleteId; //親要素のdomから、削除するノードクラスの位置を検索する
    ...
    palent.child = palent.child.filter(function (child) {
      return (child.dir === node.dir && child.id === node.id) === false;
    }); //ノードクラスのデータを削除した後に、ツリーの構造が変わるので、css名を変更する

    chainparser.decisionChildClass(palent, palentPalent); //削除するノードクラスのdomを削除する
    ...
    if (palent.child.length === 0) {
      ...
      for (var _i5 = 0; _i5 < palentPalent.child.length; _i5++) {
        if (chainparser.isEqual(palentPalent.child[_i5], palent)) {
          deleteId = _i5;
        }
      } //親要素の親要素から、親要素を削除する


      palentPalent.deleteElement(deleteId); //親要素のdomを作成する

      palent.createElement(); //投影元があるなら、タイトルを斜体にする
      ...
      var insertElement; //expandtree系の場合
      ...
      if (palentPalent.className === 'expandtree' || palentPalent.className === 'lastexpandtree') {
        ...
        //chaintreeの場合
        //ひとつ前のdomを取得する
        insertElement = palentPalent.element.children[deleteId - 1]; //ひとつ前のdomから後ろに挿入する
      ...
      //ノードクラスをdomから削除する
      palent.deleteElement(deleteId); //親ノードの子要素のcssを変更している部分をdomに反映させる。
...
TreeAction.paste = function (Node, chainparser, tree) {
  ...
  var pasteNode = function pasteNode(toNodeDir, toNodeId, fromNodeDir, fromNodeId, fromNodeChain) {
    //@var Nodeクラス toNode 貼付先のノードクラス
    var toNode = chainparser.searchNodeDirId(toNodeDir, toNodeId, tree); //@var Nodeクラス fromNode 貼付元のノードクラスのコピー

    var fromNode = chainparser.searchNodeDirId(fromNodeDir, fromNodeId, tree).deepCopyNode(); //コピーしたクラスのディレクトリを更新する

    chainparser.recreateDir(fromNode, toNodeDir); //上下関係のオブジェクトのデータがある場合について、idを更新する
    //移動の機能時は、idを更新しない
    //idを更新する
    ...
    var concatFromNode = chainparser.concatNode(fromNode);

    for (var i = 0; i < concatFromNode.length; i++) {
      //@var array fromLink 投影元のディレクトリデータ
      var fromLink = concatFromNode[i].fromLink[0];

      if (fromLink) {
        //投影先を斜体にする
        concatFromNode[i].onSync(); //@var Nodeクラス formLinkNode 投影元のノードクラス

        var fromLinkNode = chainparser.searchNodeId(fromLink, tree); //投影元のノードクラスに、貼付先のデータを追加する
  ...
  var appendFromNode = function appendFromNode(toNode, fromNode) {
    //@var array twoPalent 貼付先の親ノードと親の親ノードの配列
    var twoPalent = chainparser.getTwoPalent(toNode, tree); //@var Nodeクラス 貼付先の親ノード

    var palent = twoPalent[0]; //@var Nodeクラス 貼付先の親の親ノード

    var palentPalent = twoPalent[1]; //親からclassNameを決め直す

    chainparser.decisionChildClass(palent, palentPalent); //貼付先にdomを追加する
    //親要素のdom要素の削除と再構成
    ...
    if (toNode.child.length === 1) {
      ...
      palent.deleteElement(deleteId); //貼付先のdomを展開するボックスに作り直す

      toNode.createExpandTree(); //貼付先に、コピーした移動元のdomを追加する

      toNode.appendTree(fromNode.element); //貼付先の子要素のcss名を、domに反映させる

      toNode.resettingClassElement(); //新しく作り直した貼付先のdomを、親ノードのdomに追加する

      palent.insertTree(toNode.element, deleteId); //親ノードに新しくdomを追加したので、
      ...
      //貼付先のボックスを開く。
      toNode.openBox(); //貼付先の子要素を表示する

      toNode.openDisplayNode(); //コピーした貼付元のdomを追加する
  ...
  var pasteMyTreeNode = function pasteMyTreeNode(toNodeDir, toNodeId, fromNodeChain) {
    //@var Object chain 上下関係のオブジェクトのデータ
    var chain = fromNodeChain[0]; //@var Nodeクラス toNode マイツリーのNodeクラス

    var toNode = chainparser.searchNodeDirId(toNodeDir, toNodeId); //@var string appendDir マイツリーに登録するノードのディレクトリ

    var appendDir = '/' + Object.keys(chain)[0].split('.')[1] + '/' + Object.values(chain)[0].split('.')[1]; //@var string appendId マイツリーに登録するノードのid

    var appendId = Object.values(chain)[0].split('.')[0]; //@var Nodeクラス appendNode マイツリーに登録するノードクラス

    var appendNode = new Node(appendDir, appendId); //マイツリーの子要素のcss名はsecondtree

    appendNode.className = "secondtree"; //登録するノードクラスのdomを作成し、マイツリーに挿入する
...
TreeAction.move = function (chainparser, tree, pasteFunction, deleteFunction) {
  ...
  //ツリーのプログラムの移動機能
  var moveNode = function moveNode(toNodeDir, toNodeId, fromNodeDir, fromNodeId) {
    //@var Nodeクラス toNode 移動先のノードクラス
    var toNode = chainparser.searchNodeDirId(toNodeDir, toNodeId, tree); //@var Nodeクラス formNode 移動元のノードクラス

    var fromNode = chainparser.searchNodeDirId(fromNodeDir, fromNodeId, tree); //@var array fromTwoPalent 移動元のノードクラスと親要素(0番目)と親要素の親要素(1番目)のクラスの配列

    var fromTwoPalent = chainparser.getTwoPalent(fromNode, tree); //@var Nodeクラス fromPalent 移動元の親要素のクラス

    var fromPalent = fromTwoPalent[0]; //移動先の下に、貼り付けるので移動先のと移動元の親クラスが同じ場合
...
TreeAction.projection = function (chainparser, tree) {
  ...
  var projectionNode = function projectionNode(toNodeDir, toNodeId, fromNodeDir, fromNodeId, fromNodeChain) {
    //@var Nodeクラス toNode 投影先のノードクラス
    var toNode = chainparser.searchNodeDirId(toNodeDir, toNodeId, tree); //@var Nodeクラス copyFromNode 投影元のノードクラスをコピーしたノードクラス

    var fromNode = chainparser.searchNodeDirId(fromNodeDir, fromNodeId, tree).deepCopyNode(); //コピーしたノードクラスのディレクトリを投影先のディレクトリで更新する

    chainparser.recreateDir(fromNode, toNodeDir); //idを更新する
    ...
    chainparser.syncLinkNode(fromNodeId, fromNode, tree); //投影先を斜体にする

    fromNode.sync(); //投影先に、コピーしたノードを追加する

    toNode.child.push(fromNode); //投影先に投影元のdomを追加する
  ...
  var appendFromNode = function appendFromNode(toNode, fromNode) {
    ...
    var twoPalent = chainparser.getTwoPalent(toNode, tree); //@var Nodeクラス palent 投影先の親ノード

    var palent = twoPalent[0]; //@var Nodeクラス palentPalent 投影先の親の親ノード
    ...
    if (toNode.child.length === 1) {
      ...
      palent.deleteElement(deleteId); //投影先のdomを展開するボックスに作り直す

      toNode.createExpandTree(); //投影先に、コピーした移動元のdomを追加する

      toNode.appendTree(fromNode.element); //投影先の子要素のcss名を、domに反映させる

      toNode.resettingClassElement(); //新しく作り直した投影先のdomを、親ノードのdomに追加する
      ...
      //投影先のボックスを開く。
      toNode.openBox(); //投影先の子要素を表示する

      toNode.openDisplayNode(); //コピーした投影元のdomを追加する
...
TreeAction.append = function (Node, chainparser, tree) {
  ...
  var nodeAppend = function nodeAppend(toNodeDir, toNodeId, fromNodeChain) {
    //@var Nodeクラス 追加先のノードクラス
    var toNode = chainparser.searchNodeDirId(toNodeDir, toNodeId, tree); //@var object 上下関係のオブジェクトのデータ

    var chain = fromNodeChain[0]; //@var Nodeクラス 追加するノードクラス

    var fromNode = new Node(toNodeDir + "/" + Object.values(chain)[0].split('.')[1], Object.values(chain)[0].split('.')[0]); //プロトタイプに、チェインパーサーのクラスとツリーインスタンスを追加する
    ...
    toNode.child.push(fromNode); //追加ノードのdomを作成する
  ...
  var appendFromNode = function appendFromNode(toNode, fromNode) {
    ...
    fromNode.createLeafTree(); //貼付先にdomを追加する
    //親要素のdom要素の削除と再構成
    //ノードクラスを追加した時に、投影先の子要素が1の場合
    //新しく子要素ができた事になるので、投影先を展開するボックスのあるツリーに変更する
...
TreeAction = function (treesepalete, projectionChain) {
  ...
  var tree = TreeAction.createTree(treesepalete, projectionChain, Node, chainparser); //@var nodeDeleteクラス 削除機能クラス

  var deleteFunction = TreeAction["delete"](chainparser, tree); //@var nodePasteクラス 貼付機能クラス

  var pasteFunction = TreeAction.paste(Node, chainparser, tree); //@var nodeMoveクラス 移動機能クラス

  var moveFunction = TreeAction.move(chainparser, tree, pasteFunction, deleteFunction); //@var nodeProjectionクラス 投影機能クラス

  var projectionFunction = TreeAction.projection(chainparser, tree); //@var nodeAppend 追加機能クラス

  var appendFunction = TreeAction.append(Node, chainparser, tree); //クリップボードのデータを保存する

  var copyNode = function copyNode() {
    //マイツリーのノードクラスるか判断する
    if (checkLineTree(_ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getSelectDir()) === true) {
      //@var Nodeクラス node 本来のノードクラス
      var node = searchNodeId(_ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getSelectId()); //マイツリーのノードクラスの場合、本来のノードクラスのディレクトリとidをコピーする

      _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.copyNode(node.dir, node.id);
    } else {
      //選択したノードクラスのディレクトリとidをコピーする
      _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.copyNode(_ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getSelectDir(), _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getSelectId());
    }
  }; //ノードを削除する
  ...
  var deleteNode = function deleteNode() {
    ...
    var selectNodeDir; //@var string selectNodeId 選択したノードのid

    var selectNodeId; //@var Nodeクラス 削除するノードクラス

    var node; //選択したノードがマイツリーにあるかチェックする

    if (checkLineTree(_ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getSelectDir()) === true) {
      //マイツリーのノードは、本来のデータが置いてあるノードとは違うので、本来のノードを検索する。
      //@var Nodeクラス node マイツリーに登録してあるノードの本来のノードクラス
      node = chainparser.searchNodeId(_ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getSelectId(), tree);
      selectNodeDir = node.dir;
      selectNodeId = node.id;
    } else {
      //選択したノードのidとディレクトリを代入する
      selectNodeDir = _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getSelectDir();
      selectNodeId = _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getSelectId();
      node = chainparser.searchNodeId(selectNodeId, tree);
    } //@var Nodeクラス カレントにする親ノード


    var palent = chainparser.getTwoPalent(node, tree)[0]; //ノードを削除する

    deleteFunction.deleteNode(selectNodeDir, selectNodeId); //親ノードをカレントにする

    palent.focus(); //選択したノードのデータを削除

    _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.select(null, null); //親ノードのデータをカレントに保存する

    _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.current(palent.dir, palent.id);
  }; //特異な操作の削除：ツリーに表示されていないノードを削除
  //@param string nodeId 削除するノードのid


  var blindDeleteNode = function blindDeleteNode(nodeId) {
    //@var Nodeクラス 削除するノードクラス
    var node = chainparser.searchNodeId(nodeId, tree); //@var Nodeクラス カレントにする親ノード

    var palent = chainparser.getTwoPalent(node, tree)[0]; //削除するノードを選択したする

    _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.select(node.dir, node.id); //ノードを削除する
  ...
  var pasteNode = function pasteNode(fromNodeChain) {
    //@var string ディレクトリ
    var dir; //@var string selectNodeDir 選択したのノードのディレクトリ

    var selectNodeDir; //@var string selectNodeId 選択したノードのid

    var selectNodeId; //選択したノードがマイツリーにあるか判断する

    if (checkLineTree(_ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getSelectDir()) === true) {
      //@var Nodeクラス node マイツリーにあるノードクラスとは違う本来のノードクラス
      var node = searchNodeId(_ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getSelectId());
    ...
    if (dir === "マイツリー") {
      ...
      //同じツリーノードにあるか判断する。
      if (checkSameTree(selectNodeDir, _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getCopyDir()) === true) {
        //同じ場合本来のノードクラスに貼り付ける
        pasteFunction.pasteNode(selectNodeDir, selectNodeId, _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getCopyDir(), _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getCopyId(), fromNodeChain);
      } else {
        //作業定義から作業指示に、貼付するか判断する
        if (getTopDir(selectNodeDir).title.slice(0, 4) === "作業指示" && TreeAction.getTopDir(_ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getCopyDir()).title.slice(0, 4) === "作業定義") {
          //本来のノードクラスに貼り付ける
          pasteFunction.pasteNode(selectNodeDir, selectNodeId, _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getCopyDir(), _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getCopyId(), fromNodeChain);
        }
      }
  ...
  var blindPasteNode = function blindPasteNode(toNodeId, fromNodeId, fromNodeChain) {
    //@var Nodeクラス 貼付先のノードクラス
    var toNode = chainparser.searchNodeId(toNodeId, tree); //@var Nodeクラス 貼付元のノードクラス

    var fromNode = chainparser.searchNodeId(fromNodeId, tree); //貼付元のノードのデータをコピーする

    _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.copyNode(fromNode.dir, fromNode.id); //貼付先のノードのデータを選択する

    _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.select(toNode.dir, toNode.id); //貼付先のノードをカレントにする

    _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.current(toNode.dir, toNode.id); //貼付する

    pasteNode(fromNodeChain); //貼付先をカレントにする

    toNode.focus(); //貼付先のノードを開く

    toNode.openBottomUpTree(); //移動によって子ノードクラスが開いていたら、閉じる

    toNode.noneDisplayTree(); //ボックスを閉じる
  ...
  var moveNode = function moveNode() {
    ...
    var node; //@var string selectNodeDir 選択したのノードのディレクトリ

    var selectNodeDir; //@var string selectNodeId 選択したノードのid

    var selectNodeId; //マイツリーのノードを選択したか判断する
    ...
    if (checkSameTree(selectNodeDir, _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getCopyDir()) === true) {
      moveFunction.moveNode(selectNodeDir, selectNodeId, _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getCopyDir(), _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getCopyId());
    }

    _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.copyNode(null, null);
    _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.current(selectNodeDir, selectNodeId);
    node.focus();
  ...
  var blindModeNode = function blindModeNode(toNodeId, fromNodeId) {
    //@var Nodeクラス 移動先のノードクラス
    var toNode = chainparser.searchNodeId(toNodeId, tree); //@var Nodeクラス 移動元のノードクラス

    var fromNode = chainparser.searchNodeId(fromNodeId, tree); //移動元のノードのデータをコピーする

    _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.copyNode(fromNode.dir, fromNode.id); //移動先のノードのデータを選択する

    _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.select(toNode.dir, toNode.id); //移動する
  ...
  var projectionNode = function projectionNode(fromNodeChain) {
    //@var string selectNodeDir 選択したのノードのディレクトリ
    var selectNodeDir; //@var string selectNodeId 選択したノードのid

    var selectNodeId; //マイツリーにある判断する
    ...
    if (checkSameTree(selectNodeDir, _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getCopyDir()) === true) {
      projectionFunction.projectionNode(selectNodeDir, selectNodeId, _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getCopyDir(), _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getCopyId(), fromNodeChain);
    }
  ...
  var blindProjectionNode = function blindProjectionNode(toNodeId, fromNodeId, fromNodeChain) {
    //@var Nodeクラス 投影先のノードクラス
    var toNode = chainparser.searchNodeId(toNodeId, tree); //@var Nodeクラス 投影元のノードクラス

    var fromNode = chainparser.searchNodeId(fromNodeId, tree); //投影元のノードのデータをコピーする

    _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.copyNode(fromNode.dir, fromNode.id); //投影先のノードのデータを選択する

    _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.select(toNode.dir, toNode.id); //投影先のノードをカレントにする

    _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.current(toNode.dir, toNode.id); //投影する

    projectionNode(fromNodeChain); //投影先をカレントにする

    toNode.focus(); //投影先のノードを開く

    toNode.openBottomUpTree(); //投影によって子ノードクラスが開いていたら、閉じる
  ...
  var appendNode = function appendNode(fromNodeChain) {
    //@var Nodeクラス node 追加先のノードクラス
    var node = chainparser.searchNodeDirId(_ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getSelectId(), _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getSelectDir(), tree);
    appendFunction.nodeAppend(_ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getSelectDir(), _ptcmcb__WEBPACK_IMPORTED_MODULE_0__.clipboard.getSelectId(), fromNodeChain);
  ...
  var current = function current(nodeId) {
    //@var Nodeクラス currentNode カレントノード
    var currentNode = chainparser.searchNodeId(String(nodeId), tree); //カレントノードを太字にする

    currentNode.focus(); //カレントノードを開く

    currentNode.openBottomUpTree(); //クリップボードの選択ノードをカレントノードにする
  ...
  var searchNodeId = function searchNodeId(nodeId) {
  ...
  var addNodeClickEvent = function addNodeClickEvent(callback) {
    ...
    var normaltreeArray = document.getElementsByClassName('normaltree'); //@var array lasttreeのdom
    ...
    if (callback) {
      ...
      for (var _i6 = 0; _i6 < firsttreeArray.length; _i6++) {
        //@var string firsttreeのディレクトリ
        var firsttreeDir = getTitleboxDir(firsttreeArray[_i6]) + '/' + firsttreeArray[_i6].children[0].innerText; //@var Nodeクラス クリック処理を追加するノード


        var _node = chainparser.searchNodeDir(firsttreeDir, tree); //idを引数にして、クリックイベントを追加する


        firsttreeArray[_i6].children[0].addEventListener('click', {
          id: _node.id,
          title: _node.title,
          handleEvent: callback
        });
      ...
      for (var _i7 = 0; _i7 < normaltreeArray.length; _i7++) {
        //@var string normaltreeのディレクトリ
        var normaltreeDir = getTitleboxDir(normaltreeArray[_i7]) + '/' + normaltreeArray[_i7].children[0].innerText; //@var Nodeクラス クリック処理を追加するノード


        var _node2 = chainparser.searchNodeDir(normaltreeDir, tree); //idを引数にして、クリックイベントを追加する


        normaltreeArray[_i7].children[0].addEventListener('click', {
          id: _node2.id,
          title: _node2.title,
          handleEvent: callback
        });
      ...
      for (var _i8 = 0; _i8 < lasttreeArray.length; _i8++) {
        //@var string lasttreeのディレクトリ
        var lasttreeDir = getTitleboxDir(lasttreeArray[_i8]) + '/' + lasttreeArray[_i8].children[0].innerText; //@var Nodeクラス クリック処理を追加するノード


        var _node3 = chainparser.searchNodeDir(lasttreeDir, tree); //idを引数にして、クリックイベントを追加する


        lasttreeArray[_i8].children[0].addEventListener('click', {
          id: _node3.id,
          title: _node3.title,
          handleEvent: callback
        });
      ...
      for (var _i9 = 0; _i9 < secondtreeArray.length; _i9++) {
        //@var string secondtreeのディレクトリ
        var secondtreeDir = getLinetreeDir(secondtreeArray[_i9]); //@var Nodeクラス クリック処理を追加するノード

        var _node4 = chainparser.searchNodeDir(secondtreeDir, tree); //idを引数にして、クリックイベントを追加する


        secondtreeArray[_i9].children[0].addEventListener('click', {
          id: _node4.id,
          title: _node4.title,
          handleEvent: callback
        });
      ...
      for (var _i10 = 0; _i10 < endtreeArray.length; _i10++) {
        //@var string endtreeのディレクトリ
        var endtreeDir = getLinetreeDir(endtreeArray[_i10]); //@var Nodeクラス クリック処理を追加するノード

        var _node5 = chainparser.searchNodeDir(endtreeDir, tree); //idを引数にして、クリックイベントを追加する


        endtreeArray[_i10].children[0].addEventListener('click', {
          id: _node5.id,
          title: _node5.title,
          handleEvent: callback
        });
  ...
  var checkLineTree = function checkLineTree(nodeDir) {
    //@var string topDir ディレクトリの第一階層のタイトル("部署")
    var topDir = getTopDir(nodeDir); //linetreeにあるか判断する
  ...
  var checkSameTree = function checkSameTree(toNodeDir, fromNodeDir) {
    //@var string toTopDir 第一階層のノードのタイトル
    var toTopDir = getTopDir(toNodeDir); //var sring fromToDir 第一階層のノードのタイトル

    var fromTopDir = getTopDir(fromNodeDir); //第一階層のタイトルが等しいなら、同じツリー内部にある
  ...
  var getTopDir = function getTopDir(nodeDir) {
    //@var array splitToDir ディレクトリの文字を分割した配列
    var splitToDir = nodeDir.split('/'); //先頭の空を削除する
