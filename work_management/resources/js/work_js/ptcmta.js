...
TreeAction.node = class Node {
  ...
  getImg(){
    ...
    src = `<img width = "15" length = "17" src = "data:image/png;base64,${base64ImgList[src]}">`;
...
TreeAction = ((treesepalete, projectionChain) => {
  ...
  let scrollNode = function scrollNode(node){
    ...
    //@var dom id=treeのdom
    let treeTag = document.getElementById('tree');
    //@var int ツリー画面の表示領域とノードの位置の差(width)
    let left = (treeTag.getBoundingClientRect().left + treeTag.clientWidth)*0.6 - node.element.getBoundingClientRect().left;
    //@var int ツリー画面の表示領域とノードの位置の差(height)
    let top = (treeTag.getBoundingClientRect().top + treeTag.clientHeight)*0.6 - node.element.getBoundingClientRect().top;
  ...
  //一覧表示からのノードの表示
  if(document.location.pathname.split('/')[1] === 'show' || document.location.pathname.split('/')[2] === 'show'){
    
    //@var Nodeクラス 開くノード
    let node;

    //@var array パスをスラッシュで区切った配列
    let pathArray = document.getElementById('copyTarget').parentNode.action.split('/');
    //@var string 詳細行の部署のid
    let nodeId = pathArray[pathArray.length - 1];
