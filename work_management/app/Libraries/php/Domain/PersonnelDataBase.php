<?php
    ...
    class PersonnelDataBase {
        ...
        public static function get($client,$select_id){
            ...
            $data = DB::select('select dcji01.client_id,personnel_id,name,email,
            password,password_update_day,status,management_personnel_id,login_authority,
            system_management,operation_start_date,operation_end_date,remarks,
            dcji01.created_at,dcji01.updated_at ,high_id ,lower_id,
            (select dcbs01.name from dcji01 
            inner join dccmks on dcji01.personnel_id = dccmks.lower_id
            inner join dcbs01 on dcbs01.department_id = dccmks.high_id
            where dcji01.client_id = ? and dcji01.personnel_id = ?) as high_name
            from dcji01 inner join dccmks 
            on dcji01.personnel_id = dccmks.lower_id 
            where dcji01.client_id = ? and dcji01.personnel_id = ?',
            [$client,$select_id,$client,$select_id]);
